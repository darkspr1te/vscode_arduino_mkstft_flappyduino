// SPIflash_FileListWithImageSize 
// Copyright (C)2019 Rinky-Dink Electronics, Henning Karlsen. All right reserved
// web: http://www.RinkyDinkElectronics.com/
//
// This program will attempt to read the list of files
// from the flash memory chip.
//
// The chip must contain the SPIflash file system.
//

#include <SPIflash.h>

// Software SPI
// Remember to set the correct pins for your development board
// Parameter order: SI/MOSI, SO/MISO, SCK/SCLK, CE/SS
//SPIflash myFlash(5, 6, 7, 4);

// Hardware SPI
// Remember to set the correct pin for your development board
// Parameter order: CE/SS
SPIflash myFlash(53);

uint16_t fileid = 0;
uint16_t filetype = 0;
uint32_t filesize = 0;
uint16_t bitrate = 0;
uint16_t bps = 0;
uint16_t channels = 0;
float    playtime = 0;
char buf[100];

void setup()
{
  Serial.begin(115200);
  myFlash.begin();
}

void loop()
{
  if (myFlash.ID_device!=0)
  {
    sprintf(buf, "The connected device is a %s, which is a %dMbit %s from %s.", myFlash.Text_device, myFlash.Capacity, myFlash.Text_type, myFlash.Text_manufacturer);
    Serial.println(buf);
    Serial.println();
    Serial.println("| ID\t| Filetype\t\t\t\t| Filesize\t| Bitrate\t| BPS\t| Chan\t| Time\t| Note");
    Serial.println("|-------|---------------------------------------|---------------|---------------|-------|-------|-------|---------------");

    while (filetype != ERR_FILE_DOES_NOT_EXIST)
    {
      filetype = myFlash.getFileType(fileid);
      if (filetype != ERR_FILE_DOES_NOT_EXIST)
      {
        Serial.print("| ");
        sprintf(buf, "%3d", fileid);
        Serial.print(buf);
        Serial.print("\t| ");
        switch (filetype)
        {
        case 1:
          Serial.print("Binary\t\t\t\t| ");
          break;
        case 2:
          Serial.print("Text\t\t\t\t\t| ");
          break;
        case 3:
          Serial.print("Text Resource\t\t\t\t| ");
          break;
        case 4:
          Serial.print("Color image\t\t\t\t| ");
          break;
        case 5:
          Serial.print("Monochrome image for color display\t| ");
          break;
        case 6:
          Serial.print("Monochrome image\t\t\t| ");
          break;
        case 7:
          Serial.print("Audio\t\t\t\t\t| ");
          break;
        case 32:
        case 33:
        case 34:
        case 35:
        case 36:
        case 37:
        case 38:
        case 39:
        case 40:
        case 41:
        case 42:
        case 43:
        case 44:
        case 45:
        case 46:
        case 47:
          Serial.print("Custom ");
          Serial.print(filetype-31, DEC);
          Serial.print("\t\t\t\t| ");
          break;
        default:
          Serial.print("<unknown>\t\t\t\t| ");
          break;
        }
        filesize = myFlash.getFileSize(fileid);
        sprintf(buf, "%13lu", filesize);
        Serial.print(buf);
        Serial.print("\t| ");
        if (filetype == 7)
        {
          bitrate = myFlash.getAudioSamplerate(fileid);
          bps = myFlash.getAudioBPS(fileid);
          channels = myFlash.getAudioChannels(fileid);
          playtime = ((((float)filesize / (bps / 8)) / channels) / bitrate);
          sprintf(buf, "%13d", bitrate);
          Serial.print(buf);
          Serial.print(" | ");
          sprintf(buf, "%5d", bps);
          Serial.print(buf);
          Serial.print(" | ");
          sprintf(buf, "%5d", channels);
          Serial.print(buf);
          Serial.print(" | ");
          dtostrf(playtime, 5, 1, buf);
          Serial.print(buf);
          Serial.print(" | ");
        }
        else
        {
          Serial.print("\t\t|\t|\t|\t| ");
        }
        myFlash.readFileNote(fileid, buf);
        if (buf[0] == 0)
          Serial.print("<no note>\t");
        else
          Serial.print(buf);
        Serial.println();
        fileid++;
      }
    }
    Serial.println("|-------|---------------------------------------|---------------|---------------|-------|-------|-------|---------------");
  }
  else
    Serial.println("Unknown flash device!");
  
  while(1){};
}
